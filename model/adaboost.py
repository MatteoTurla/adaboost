...
class BinaryAdaBoost:
    ...
    class is implemented to be able to works also with other base classifier 
    ...
    -------
    fit(X,y):
        train adaboost classifier on X,y
        return self
    predict(X)
        return sign of the weighted binary prediction
    decision_function(X) 
        return the weighted sum of binary base prediciton
    clone()
        return a new instane of the current class
    score(X, y)
